...
def test_should_accept_age_as_char_field_and_coordinates_as_json():
    # weltenhub-Modelle (main-fa00e9c): Character.age = CharField ("mid-30s", "" bei
    # blank), Location.coordinates = JSONField (dict oder null). Realfall 2026-08-14:
    # characters.create() starb an age="" — zweiter Lauf desselben Strangs.
    basis = {
        "id": str(uuid.uuid4()),
        "world": str(uuid.uuid4()),
        "name": "Erik",
    ...
    assert CharacterSchema.model_validate({**basis, "age": ""}).age == ""
    assert CharacterSchema.model_validate({**basis, "age": "mid-30s"}).age == "mid-30s"
    ort = LocationSchema.model_validate(
